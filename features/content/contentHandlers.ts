...
import { jsonUnauthorized, jsonForbidden, jsonError, jsonTooManyRequests } from '@/shared/response';
...
export async function incidentEventsCreateHandler(req: NextRequest) {
  ...
  // Typed event ID ("SY" — system fault) shown to the user and admins so the
  // crash can be referenced in support requests. Stored in metadata so the
  // admin console can resolve it back to this incident.
  ...
  const event = await prisma.incident_events.create({
    data: {
      ...
      metadata: { ...metadata, eventId: syEventId },
    },
  ...
  return NextResponse.json({ event, eventId: syEventId }, { status: 201 });
}
...
async function notifyAdminsOfCrash(event: any, data: {
  ...
  try {
    ...
    const { getAdminBaseUrl } = await import('@/config/app-urls');
    const adminBase = process.env.ADMIN_URL || getAdminBaseUrl();
