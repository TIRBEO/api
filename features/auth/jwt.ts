...
/**
 * Short-lived re-authentication proof (sensitive actions like passkey removal).
 * Issued only after a successful passkey assertion with userVerification, and
 * checked by destructive handlers so a stolen session cookie alone cannot
 * revoke credentials.
 */
export async function signReauthToken(userId: string): Promise<string> {
  return new SignJWT({ sub: userId, purpose: 'passkey-reauth' })
    .setProtectedHeader({ alg: 'HS256' })
    .setIssuedAt()
    .setExpirationTime('5m')
    .sign(getSecret());
...
export async function verifyReauthToken(token: string): Promise<string | null> {
  try {
    const { payload } = await jwtVerify(token, getSecret(), { algorithms: ['HS256'] });
    if (payload.sub && payload.purpose === 'passkey-reauth') return payload.sub as string;
    return null;
  } catch {
    return null;
  }
...
export async function signPendingSignupToken(data: {
  ...
  name?: string;
  photoUrl?: string;
  /** Optional post-signup redirect target (validated by isAllowedRedirect). */
  redirect?: string;
}): Promise<string> {
...
export async function verifyPendingSignupToken(
  token: string,
): Promise<{ provider: string; providerId: string; email: string; name?: string; photoUrl?: string; redirect?: string } | null> {
  try {
    const { payload } = await jwtVerify(token, getSecret(), { algorithms: ['HS256'] });
    ...
    return {
      ...
      photoUrl: (payload.photoUrl as string) || undefined,
      redirect: (payload.redirect as string) || undefined,
    };
  } catch {
    return null;
  }
