import { recordRateLimitHit } from '@/features/auth/suspicious-activity';
...
function startAlertChecker(): void {
  if (alertCheckTimer || process.env.VERCEL) return;
  function scheduleAlertCheck() {
    alertCheckTimer = setTimeout(() => {
...
// Start on module load (skip on Vercel)
if (!process.env.VERCEL) startAlertChecker();
