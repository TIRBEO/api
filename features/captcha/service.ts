...
// 30s TTL — block/unblock invalidate the affected keys explicitly
// (invalidateBlockedCache), so a longer window is safe and avoids up to 3 DB
// round-trips (user + session + ip) on every state-changing request.
...
async function notifyAdminOfBlock(userId: string | undefined, sessionId: string | undefined, ipAddress: string | undefined, reason: string, rayId: string, warningCount: number): Promise<void> {
  try {
    const { notifyAdmins } = await import('@/features/security/security');
    await notifyAdmins({
