...
import { generateChallenge, CHALLENGE_TYPES } from '@/features/captcha/challenges';
...
function signToken(fields: { id: string; exp: number; nonce: string; ipHash: string; uaHash: string; fpHash: string }): string {
  ...
  if (!secret) throw new Error("CAPTCHA_TOKEN_SECRET or JWT_SECRET env var is required");
