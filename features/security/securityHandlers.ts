...
import { revokeSessionFamilyByUser, bustSessionCache } from '@/features/auth/session';
...
import { createNotification, NotifType } from '@/features/notifications/notifications';
...
export async function totpDisableHandler(request: NextRequest) {
  try {
    ...
    const totpCode: string | undefined =
      proof.method === 'totp'
        ? (normalizeTotpCode(proof.body?.totpCode || proof.body?.code || request.nextUrl?.searchParams.get('code')) || undefined)
        : undefined;
    if (!totpCode) {
      return NextResponse.json({ error: 'Invalid code. Enter a 6-digit code.' }, { status: 400 });
    }
...
export async function recoveryEmailHandler(request: NextRequest) {
  try {
    ...
    // Removing the recovery email is a sensitive action — require an identity
    // proof (passkey assertion, password, or TOTP). Adding/changing keeps its
    // own email-OTP verification flow below.
...
export async function sessionsRevokeAllHandler(request: NextRequest) {
  try {
    ...
    // or TOTP). A stolen session cookie alone cannot sign out every device.
    const proof = await requireReauth(request, session.userId);
    if ('response' in proof) return proof.response;
