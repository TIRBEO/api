...
import { detectXss, sanitizeInput } from '@/features/security/security';
...
// Run cleanup every 5 minutes (skip on Vercel — each invocation is independent)
...
if (typeof setTimeout !== 'undefined' && !process.env.VERCEL) {
  const gFormSec = globalThis as any;
  if (!gFormSec.__tirbeoFormSecCleanupRunning) {
    gFormSec.__tirbeoFormSecCleanupRunning = true;
    (function scheduleFormSecCleanup() {
      gFormSec.__tirbeoFormSecCleanupTimer = setTimeout(() => {
        cleanupRateLimits();
        scheduleFormSecCleanup();
      }, 5 * 60 * 1000);
    })();
  }
}
