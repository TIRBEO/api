...
import { buildTemplates } from '@/features/email/email-templates';
...
export async function sendEmail(
  ...
  subject: string,
  htmlBody: string,
  options?: { fromEmail?: string; fromName?: string; replyTo?: string; threadId?: string; templateName?: string; metadata?: Record<string, unknown>; categoryOverride?: string }
): Promise<EmailResult> {
  ...
  if (options?.templateName) {
    try {
      ...
      if (await shouldSuppressEmail(to, options.templateName, options.categoryOverride)) {
...
export async function sendTemplateEmail(
  ...
  options?: { fromEmail?: string; fromName?: string; rawVars?: string[]; replyTo?: string; threadId?: string; skipSuppression?: boolean; categoryOverride?: string }
): Promise<EmailResult> {
  ...
  if (!options?.skipSuppression) {
    try {
      const { shouldSuppressEmail } = await import('@/features/email/emailPrefs');
      if (await shouldSuppressEmail(to, templateName, options?.categoryOverride)) {
  ...
  try {
    const { ESSENTIAL_TEMPLATES, isBrainMandatory } = await import('@/features/email/emailPrefs');
    ...
    if (!isEssential && !isBrainSec) {
      ...
      mergedVars['unsubscribeSection'] = `<p style="margin:8px 0 0;font-size:11px;color:rgba(245,245,245,0.5)"><a href="${apiBase}/api/emails/unsubscribe" style="color:rgba(245,245,245,0.5);text-decoration:underline;">Unsubscribe</a></p>`;
    } else {
