...
// DATABASE_POOL_MAX / DATABASE_POOL_MIN let each process size its pool so a
// dev server + scripts stay under Supavisor's 20-connection session limit.
const POOL_MAX = parseInt(process.env.DATABASE_POOL_MAX || '', 10) || (process.env.NODE_ENV === 'production' ? 8 : 12);
const POOL_MIN = parseInt(process.env.DATABASE_POOL_MIN || '', 10) || (process.env.NODE_ENV === 'production' ? 3 : 4);
const POOL_CONFIG = {
...
function createPool(): Pool {
  // Runtime prefers DATABASE_URL, which by Supabase convention points at the
  // transaction pooler (:6543). Transaction mode assigns a backend per
  // transaction, so N idle client sockets cost ~0 server sessions — a dev
  // server + scripts can no longer exhaust the 20-connection session limit
  // (EMAXCONNSESSION). Session-level features (advisory locks, LISTEN/NOTIFY,
  // SET) are not used anywhere in runtime code; `pgbouncer=true` below
  // disables server-side prepared statements for PgBouncer compatibility.
  // Migrations bypass this pool entirely (prisma.config.ts uses the direct
  // session URL, where DDL and long DDL transactions are safe).
  ...
  const pool = new Pool({
    ...
    ssl: process.env.NODE_ENV !== 'production'
      ...
      : undefined,
    // TCP keepalive: probe dead sockets every 30s instead of hanging for
    // minutes when the network drops mid-request (VPN out, DNS failure).
    keepAlive: true,
    keepAliveInitialDelayMillis: 30_000,
    // Client-side deadline for any single query. During a network outage
...
function initializePoolAndPrisma(): { prisma: PrismaClient; pool: Pool } {
  ...
  // Skip on Vercel — each serverless invocation is independent, no persistent pool.
  // Uses setTimeout recursion instead of setInterval to avoid CJS bundler warnings.
  const isVercel = !!process.env.VERCEL;
  const g = globalThis as any;
  if (!isVercel && !g.__tirbeoPoolKeepAlive) {
    let keepAliveCount = 0;
    const KEEP_ALIVE_MS = 20_000;
    function scheduleKeepAlive() {
      g.__tirbeoPoolKeepAlive = setTimeout(async () => {
        keepAliveCount++;
        try {
...
// Start monitoring in production and development (skip on Vercel — no persistent process)
// Uses setTimeout recursion instead of setInterval to avoid CJS bundler warnings.
if (typeof setTimeout !== 'undefined' && !process.env.VERCEL) {
  const gMonitor = globalThis as any;
  if (!gMonitor.__tirbeoPoolMonitorRunning) {
    gMonitor.__tirbeoPoolMonitorRunning = true;
    (function scheduleMonitor() {
      gMonitor.__tirbeoPoolMonitorTimer = setTimeout(() => {
        monitorPool();
        scheduleMonitor();
      }, POOL_CHECK_INTERVAL);
    })();
