...
// Resolve an app path (or absolute URL) against the dashboard origin so the
// service worker always opens a canonical link on click.
function getAppUrl(path: string): string {
  if (/^https?:\/\//i.test(path)) return path;
  const { getDashboardBaseUrl } = require('@/config/app-urls');
...
async function getPrisma() {
  try { const { prisma } = await import('@/infrastructure/db/prisma'); return prisma; } catch { return null; }
}
...
export async function subscribeToPush(userId: string, subscription: { endpoint: string; p256dh: string; auth: string }, userAgent?: string) {
  ...
  try {
    const { hasConsent } = await import('@/features/users/consent');
