...
export async function updateReport(id: string, data: {
  ...
  const report = await prisma.contentReport.update({
    where: { id },
    data: {
      ...(data.status && { status: data.status as any, reviewedAt: new Date() }),
      ...(data.reviewedById && { reviewedBy: { connect: { id: data.reviewedById } } }),
      ...(data.action !== undefined && { actionTaken: data.action }),
