...
const p = new PrismaClient({ adapter: new PrismaPg({ connectionString: process.env.DIRECT_DATABASE_URL || process.env.DATABASE_URL }) });
...
async function main() {
  ...
  console.log('Roles:', await p.app_roles.count());
